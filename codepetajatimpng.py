...
# ===============================
USE_LOG = True  # false = ghi linier, true = log10(GHI)
...
output_dir = os.path.join(base_output_dir, "coba_peta_jatimlagilog")
...
for (tahun, periode), grup in df.groupby(["year", "periode"]):
    ...
    # Offset Untuk Nama Kabupaten Yang Eror
    LABEL_OFFSET = {
	"Gresik": (0.0, -0.15),
	"Sumenep": (0.0, -0.12),
}
    ...
    # Terapkan Jika Ada Yang Eror 
    if row["LABEL_NAME"] in LABEL_OFFSET:
        dx, dy = LABEL_OFFSET[row["LABEL_NAME"]]
        x += dx 
        y += dy
